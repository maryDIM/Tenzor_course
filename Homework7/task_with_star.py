...
class RomanNums:
    ...
    def from_roman(self):
        ...
        #     arabic_num += my_dict[i]
        for i in range(len(self.roman_num) - 1):
            if my_dict[self.roman_num[i]] >= my_dict[self.roman_num[i + 1]]:
                arabic_num += my_dict[self.roman_num[i]]
            else:
                arabic_num -= my_dict[self.roman_num[i]]

        arabic_num += my_dict[self.roman_num[-1]]
        return arabic_num
    ...
    def is_palindrome(self):
        ...
        """
        if str(self.from_roman())[:] == str(self.from_roman())[::-1]:
            return True
        else:
            return False
